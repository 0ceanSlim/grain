...
// ckindToKind maps a common-kind bucket index to its kind number. Order MUST
// match enum ndb_common_kind in nostrdb.h; each bucket is exactly one kind
// (ndb_kind_to_common_kind — "list" is kind 30000), so -1 is left to "other".
var ckindToKind = []int{0, 1, 3, 4, 5, 6, 7, 9735, 9734, 23194, 23195, 27235, 30000, 30023, 30315}

// EventStats returns the stored-event total and its distribution over nostrdb's
// common-kind buckets (Profile, Text, Contacts, …) plus an "other" bucket for
// everything else; zero-count buckets are omitted.
//
// NOT cheap: ndb_stat walks every entry of every LMDB table (~340ms on a
// 300k-event relay). Never call it per request — the dashboard endpoint
// serves a cached snapshot (server/api/relayStats.go).
func (db *NDB) EventStats() (total uint64, byKind []KindStat, err error) {
	db.mu.RLock()
	defer db.mu.RUnlock()

	if db.ndb == nil {
		return 0, nil, fmt.Errorf("nostrdb is closed")
	}

	var stat C.struct_ndb_stat
	if C.ndb_stat(db.ndb, &stat) == 0 {
		return 0, nil, fmt.Errorf("ndb_stat failed")
	}
	...
	return uint64(stat.dbs[C.NDB_DB_NOTE].count), out, nil
}

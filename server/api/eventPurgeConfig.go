...
func GetEventPurgeConfig(w http.ResponseWriter, r *http.Request) {
	log.Util().Debug("Event purge config API endpoint accessed",
	...
	if err := json.NewEncoder(w).Encode(response); err != nil {
		log.Util().Error("Failed to encode event purge config response",
			"client_ip", utils.GetClientIP(r),
			"error", err)
	...
	log.Util().Info("Event purge config served successfully",
		"client_ip", utils.GetClientIP(r))
}
...
func GenerateKeyPair() (*KeyPair, error) {
	log.Util().Debug("Generating new Nostr key pair")
	
	// Generate 32 random bytes for private key
	privateKeyBytes := make([]byte, 32)
	if _, err := rand.Read(privateKeyBytes); err != nil {
		log.Util().Error("Failed to generate random private key", "error", err)
	...
	if err != nil {
		log.Util().Error("Failed to derive public key", "error", err)
	...
	if err != nil {
		log.Util().Error("Failed to encode private key to nsec", "error", err)
	...
	if err != nil {
		log.Util().Error("Failed to encode public key to npub", "error", err)
	...
	log.Util().Info("Successfully generated Nostr key pair", 
		"pubkey", publicKeyHex,
		"npub", npub)

...
func TestGoReportCardCompliance(t *testing.T) {
	...
	t.Run("NoDeadCode", func(t *testing.T) {
		// main.go embeds docs/openapi/swagger.json via //go:embed, which
		// is generated by `swag init` at build/release time (see
		// tests/docker/Dockerfile and the release workflow). The file is
		// .gitignored so it isn't checked in. When this review test runs
		// host-side (under CI's `make test-run`, not inside the docker
		// build container), the spec hasn't been generated yet and
		// `go build ./...` fails with `pattern docs/openapi/swagger.json:
		// no matching files found`.
		//
		// Generate a minimal stub if the file is missing so the compile
		// check exercises real code, not a missing-embed quirk. A real
		// spec is regenerated by the docker build step that actually
		// ships; the stub here exists only for this test's go-build to
		// succeed.
		specPath := filepath.Join(projectRoot, "docs/openapi/swagger.json")
		if _, err := os.Stat(specPath); os.IsNotExist(err) {
			if err := os.MkdirAll(filepath.Dir(specPath), 0o755); err != nil {
				t.Fatalf("create openapi dir: %v", err)
			}
			stub := []byte(`{"swagger":"2.0","info":{"title":"grain test stub","version":"dev"},"paths":{}}`)
			if err := os.WriteFile(specPath, stub, 0o644); err != nil {
				t.Fatalf("write stub swagger.json: %v", err)
			}
			t.Logf("generated stub %s for compile check", specPath)
		}
